...
function UnlinkWaitingContent() {
  ...
  React.useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          clearInterval(timer)
          router.push("/bidv-link/unlink-result?state=failed-timeout")
          return 0
        }
        return prev - 1
      })
  ...
  React.useEffect(() => {
    if (state === "callback-success") {
      const t = setTimeout(() => router.push("/bidv-link/unlink-result?state=success"), 1500)
      return () => clearTimeout(t)
    }
    if (state === "callback-failed") {
      const t = setTimeout(() => router.push("/bidv-link/unlink-result?state=failed"), 1500)
      return () => clearTimeout(t)
    }
    if (state === "callback-pending") {
      const t = setTimeout(() => router.push("/bidv-link/unlink-result?state=pending"), 1500)
      return () => clearTimeout(t)
    }
    if (state === "callback-cancel") {
      const t = setTimeout(() => router.push("/bidv-link/unlink-result?state=failed-cancel"), 1500)
      return () => clearTimeout(t)
    }
  }, [state, router])
  ...
  return (
    <div className="relative w-full max-w-[390px] min-h-screen bg-background text-foreground flex flex-col">
      <Header variant="default" title="Xác thực hủy liên kết" />
      ...
      <div className="flex-1 overflow-y-auto pb-[21px]">
        <div className="flex flex-col items-center justify-center px-[22px] pt-[80px]">
          ...
          <h2 className="text-lg font-semibold leading-6 tracking-[-0.005em] text-foreground text-center mb-[8px]">
            Xác thực hủy liên kết tại BIDV SmartBanking
          </h2>
      ...
      <Dialog
        ...
        title="Hủy giao dịch?"
        description="Yêu cầu hủy liên kết sẽ bị hủy."
        primaryLabel="Hủy giao dịch"
        secondaryLabel="Tiếp tục chờ"
        footerProps={{
          primaryProps: {
            intent: "danger",
            onClick: () => router.push("/bidv-link/bank-detail"),
          },
...
export default function UnlinkWaitingPage() {
  return (
    <React.Suspense fallback={null}>
      <UnlinkWaitingContent />
    </React.Suspense>
  )
}
