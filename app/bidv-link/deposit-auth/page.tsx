...
function DepositAuthContent() {
  ...
  const [storeDialog, setStoreDialog] = React.useState(state === "redirect-store")
  ...
  React.useEffect(() => {
    if (pin.length === 6 && state !== "pin-success" && state !== "pin-error-1" && state !== "pin-error-2") {
      setTimeout(() => {
        router.push("/bidv-link/deposit-waiting")
      }, 800)
    }
  }, [pin, router, state])
  ...
  React.useEffect(() => {
    if (state === "biometric-success") {
      const t = setTimeout(() => router.push("/bidv-link/deposit-waiting"), 1500)
  ...
  const handleConfirm = () => {
    if (authMethod === "pin") {
      router.push("/bidv-link/deposit-auth?state=pin-entry")
    } else {
      router.push("/bidv-link/deposit-waiting")
    }
  }
  ...
  return (
    <div className="relative w-full max-w-[390px] min-h-screen bg-secondary text-foreground flex flex-col">
          ...
          <>
            ...
            <div className="pt-[32px]">
              <div className="px-[22px]">
                ...
                {/* Redirect store */}
                {state === "redirect-store" && (
                  <p className="text-sm font-normal leading-5 text-foreground-secondary mt-[16px] text-center">
                    Đang chuyển hướng đến App Store...
                  </p>
                )}
              </div>
            </div>
      ...
      <Dialog
        ...
        footerProps={{
          primaryProps: { onClick: () => { setSessionDialog(false); router.push("/bidv-link/deposit") } },
      ...
      <Dialog
        ...
        footerProps={{
          ...
          secondaryProps: { onClick: () => { setLockedDialog(false); router.push("/bidv-link/deposit") } },
        }}
      />

      {/* Redirect Store Dialog */}
      <Dialog
        open={storeDialog}
        onClose={() => setStoreDialog(false)}
        title="Cập nhật ứng dụng"
        description="Vui lòng cập nhật ứng dụng BIDV SmartBanking để tiếp tục."
        primaryLabel="Mở App Store"
        secondaryLabel="Đóng"
        footerProps={{
          primaryProps: { onClick: () => setStoreDialog(false) },
          secondaryProps: { onClick: () => setStoreDialog(false) },
        }}
      />
...
export default function DepositAuthPage() {
  return (
